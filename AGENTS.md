...
3. **Schema constraint** — Zod LLM-raw + strict shapes; validator diagnostics: `UNSAFE_WRITE_PATH`, `WRITE_WITHOUT_USER_INTENT`, `INVALID_EXECUTION_MODE`. The compile gate also applies one deterministic rewrite: a clarification-only plan for a self-capability question ("你会做些什么" / "what can you do") is replaced by a single Commander `direct_response` answer step (`isSelfCapabilityQuestion`), because the answer lives in the runtime and asking it back returns nothing. The substitution is recorded as the `SELF_CAPABILITY_ANSWER_SUBSTITUTED` warning on the plan trace.
