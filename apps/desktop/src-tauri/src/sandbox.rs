...
pub(crate) fn run_sandboxed_command(
    ...
    if request.stdin.is_some() {
        return Err(JavisError::Validation(
            "Sandbox command stdin is not supported in the policy-only broker.".into(),
        ));
    }
    ...
    match policy.mode {
        SandboxMode::ReadOnly => run_policy_only_read_only_command(request, policy, cwd),
        SandboxMode::WorkspaceWrite => {
            ...
            Err(JavisError::Internal(
                "Workspace-write sandbox launcher is not implemented yet.".into(),
            ))
        }
...
fn platform_backend_status() -> SandboxBackendStatus {
    ...
    SandboxBackendStatus {
        backend: SandboxBackend::WindowsRestrictedToken,
        available: false,
        can_spawn: resources.ready_for_process_spawn(),
...
fn windows_backend_reason(
    ...
    missing.push("filesystem boundary enforcement is not implemented");
    missing.push("network-deny enforcement is not implemented");
    format!(
...
fn platform_backend_status() -> SandboxBackendStatus {
    ...
    SandboxBackendStatus {
        ...
        filesystem_boundary,
        network_boundary,
        reason: "Linux bubblewrap sandbox backend is not implemented yet.".to_string(),
    }
}
...
fn platform_backend_status() -> SandboxBackendStatus {
    ...
    SandboxBackendStatus {
        ...
        filesystem_boundary,
        network_boundary,
        reason: "macOS Seatbelt sandbox backend is not implemented yet.".to_string(),
    }
}
