...
- [x] **C4b** `beforeApproval` 与 `onTaskFail` 接线：审批卡现在会**汇总命中 hook 的理由**（4 个测试，含"作用域到别的工具不生效"、
  ...
  **覆盖差异照实记**：`beforeApproval` 有端到端测试，`onTaskFail` 只有 hook 单测（未构造完整的失败链路测试）- [ ] **C5** MCP 补工具搜索 / 延迟加载（把不常用工具移出前缀，顺带提升缓存命中率）
...
- [ ] **E3** 默认只给结论，点开才给证据（Inspector 信息密度过高）
- [x] **E4** 审批中心（同 D5）
- [x] **E5** 成本 / 缓存 / 上下文面板（见上方 E5 行）
- [ ] **E6** 命令面板（Cmd/Ctrl+K）
- [ ] **E7** 中断 / 续跑 / 回滚到某一步（`resolveContinuationComposeMode` 是这条的开头）
...
- [ ] **G2** 巨石拆分（见 B4）
- [ ] **G3** 文档与代码同步机制：`CLAUDE.md` 停在 2026-06-14、`CHANGELOG.md` 停在 2026-07-10、`README.md` 停在 2026-07-28，HEAD 是 2026-09-12；ReAct 描述与代码矛盾
- [ ] **G4** 仓库卫生：`.tmp-gcli-main.zip`(24 MB) / `.tmp-gcli-main/` / `.tmp-pages/` / `.dsh-tmp/` 未忽略；`docs/research/` 有价值但未提交；分支领先 origin 21 个提交 / 376 文件 / +81,676 行未合入
- [ ] **G5** 自动更新 + 版本回滚（release gate 已有，缺真实签名产物）
