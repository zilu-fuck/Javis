...
- [x] **B4c 完成：第二次尝试成功**（新增 `packages/core/src/dag-step-executors.ts`，17 个声明 / 1,403 行）：
  `workflow-executor.ts` **14,818 → 13,503 行**（**−1,315**）。这次与上次失败的区别，全靠前两轮的准备工作：
  ① 用**传递闭包**（而非人工挑选）确定要搬的正好 17 个名字；
  ② **把所有必要名字导回原文件**——上次失败正是因为只导回了函数、**没导回常量**；
...
- [ ] **B4e 判定：`workflow-executor.ts` 的可抽接缝基本用尽（数据结论）**。
  ...
  **`CommanderDagTaskOptions`（131 行）—— 整个模块的中央 options 类型**。把这个类型搬进一个"ask-user"模块，
  等于**把核心类型放进外围模块**，是设计上的错位（声明数少 ≠ 接缝合理）。
  为此给 `analyze-extraction.mjs` 加了**扇入（fan-in）**指标并设阈值 10 报警——**但它这次没有触发**：
  该类型的扇入低于 10。**这本身是个有用的发现**：它是"语义上中央"而**不是**"被广泛引用"，
  所以单靠扇入不足以识别这类风险。**阈值与信号仍需改进**（可考虑"声明体积"或"是否出现在模块入口签名里"）。
  **本轮没有切这一刀**——因为切下去会把一个中央类型放错位置，而"少一刀"远好过"错一刀"。
- [ ] **B4b/B4c 继续拆巨石的接缝已用实测选出**（`pnpm extraction:analyze`，新增 `scripts/analyze-extraction.mjs`）。  该脚本对目标文件里每个大函数统计"它引用了多少个**同文件私有声明**"——因为把一个函数搬出去是否安全，
  ...
  **⚠️ 本轮实际尝试过一次并已回滚，教训值得记（下一刀请先读这段）**：
  我用一个一次性脚本（临时文件，未提交，事后已删除）真的搬了 **17 个声明 / 1,496 行**，
...
并把它做成了可重复运行的闸门 **`pnpm roadmap:audit`**（已接入 `pnpm check`）：
...
修的是**审计脚本**（改为一次性遍历仓库、按"路径后缀"匹配），**不是路线图**——因为路线图在这些点上是对的。
这条值得记：一个会误报的审计比没有审计更危险，它会让人去"修"本来正确的东西。
...
- 2026-09-13（第 45 轮，工程）：核查下一个候选 `waitForAskUserAnswer` 后**判定不该切**——闭包虽只有 5 个声明，
  ...
  说明单独靠扇入不足以识别这类风险，阈值与信号仍需改进。**没有切这一刀**（少一刀好过错一刀）。
- 2026-09-13（第 44 轮，工程）：**B4c 搬迁成功**。新增 `dag-step-executors.ts`（17 个声明 / 1,403 行），
  `workflow-executor.ts` **14,818 → 13,503 行（−1,315）**，typecheck 0 错误、core 1,652 测试全绿（行为未变）。
  成功的关键正是前两轮的产物：传递闭包给出精确的 17 个名字；**把常量也导回原文件**（上次失败的直接原因）；
  并在移动后清理被孤立的导入。**测量 → 失败 → 根因 → 修工具 → 重做**，这条闭环走通了。
