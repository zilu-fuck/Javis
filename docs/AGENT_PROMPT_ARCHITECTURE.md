...
6. Optional Task Rules  ← 特定任务专用规则，如 UI 生成设计约束（默认不注入）
7. Custom Style         ← 用户可编辑 .md 文件，仅影响表达风格
8. Runtime Context      ← 运行时上下文，动态注入
```

前六层由代码控制，其中 Optional Task Rules 默认关闭、按任务显式打开。Custom Style 通过 `.md` 文件开放给用户。Runtime Context 由运行时动态生成。
...
packages/core/src/
  agents/
    prompt/
      ...
      uiDesignRules.ts            ← UI 生成专用设计约束（默认不注入）
...
- title: 计划标题
- reasoning: 为什么这样规划
- steps: 步骤数组，每步至少含 id / title / assignedAgentKind / successCriteria
...
### 6. Optional Task Rules（专用任务规则）

默认不注入所有 Agent，只在明确任务需要时显式打开。例如 UI 生成设计约束放在 `getUiGenerationDesignRules()`，用于未来 UI 生成 Agent 或明确的 UI 生成任务。

原因：
- 全局 prompt 保持短
- File / Research / Code 等非 UI Agent 不背前端设计规则
- 未来 UI Agent 可以复用同一段规则，不需要复制到多个 prompt

当前 UI 生成约束覆盖：
- 首屏必须是可用工作界面，不默认做营销页
- 延续现有设计系统、密度、颜色和组件
- 工具按钮、模式切换、菜单、tabs、开关等控件语义明确
- 布局有稳定尺寸和响应式约束，文本和控件不重叠、不溢出
- 表单、空态、加载、错误、禁用和成功状态可用
- UI 改动后优先跑最小相关类型检查、测试或截图验证
...
```ts
const MAX_STYLE_LENGTH = 2000; // 字符
```
...
**字数统计**：编辑器右下角显示 `{current}/{max}`（最多 2000 字符）。
...
| 5 | 超长风格文件 (>2000 字符) | 截断，Agent 正常运行 |
