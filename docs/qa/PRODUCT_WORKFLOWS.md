...
| Agent runtime durability restart-resume | A real Commander DAG persists runtime events, checkpoint state, and artifact-backed context; after packaged app restart, a restored approval card links back to the checkpoint, approval resumes downstream work, and completed upstream steps are not rerun. | Required evidence is `46-agent-runtime-restored-approval-linked.png`, `47-agent-runtime-resumed-downstream.png`, and `agent-runtime-durability-restart-qa-output.txt` showing packaged app context, runtime events persisted, checkpoint persisted, restored approval linked, approval step advanced once, upstream not rerun, downstream resumed, and artifact context restored. |
