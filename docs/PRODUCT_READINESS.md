...
- Pending confirmed-write approval recovery is not fully generalized. Durable
  ...
  remains open until shared native guards exist.
...
- Confirmed-write enforcement has separate PDF and Code Patch approval/hash
  ...
  validates approval id plus proposal hash with one-shot consumption, but a
  reusable current-file-hash layer is still needed.
...
1. Add current-file-hash guards for Code Patch and PDF confirmed writes.
