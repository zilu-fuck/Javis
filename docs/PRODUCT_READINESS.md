...
- Code Agent live opencode proposal/apply QA is not complete for real
  ...
  diagnostics. Live DeepSeek-compatible smoke failed with safe-fail (no writes
  attempted); reqwest request construction needs debugging against the working
  Python verification script.
- Task history, approval records, and workspaces still use localStorage for
  data storage. SQLite infrastructure is implemented (rusqlite + Tauri
  commands + migration system, model-settings already migrated) — remaining
  work is migrating these three localStorage consumers into the existing
  SQLite database.
...
streaming infrastructure end-to-end (Rust SSE backend → Tauri events → frontend
AsyncGenerator). The next stage is product completion:
...
2. Migrate task history, approval records, and workspaces from localStorage into
